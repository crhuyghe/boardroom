...
class CreateAccountFrame(ttk.Frame):
    ...
    def __init__(self, master=None, dark_mode=False):
        ...
        s_createAccount.configure("new.TLabel", background="#cccccc")

        # Login frame that holds text fields and buttons.
        createAccount_frame = ttk.Frame(self.master, style="new.TFrame", padding=(20, 10), relief="solid", borderwidth=1)
        createAccount_frame.pack(pady=20)

        # Title
        title_label = ttk.Label(createAccount_frame, style="new.TLabel", text="Log Into Your Account",
                                font=("Segoe UI Symbol", 16))
        title_label.pack(pady=20)
        title_label.grid(row=0, column=1, pady=5, sticky="ew")

        # Username entry field.
        ttk.Label(createAccount_frame, style="new.TLabel", text="Username:").grid(row=1, column=0, sticky="w")
        username_entry = ttk.Entry(createAccount_frame)
        ...
        login_button = FlatButton(createAccount_frame, dark_mode=False, text="Create My Account!", font=default_font,
